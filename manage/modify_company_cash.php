...
// 確認資料都有正確填入
if ($total < 0) {
    echo json_encode('廠商金的總額必須是非負整數。', JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    ...
    $link = NULL;
    echo json_encode('無法更新廠商金狀態：' . $e->getMessage(), JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    ...
    $link = NULL;
    echo json_encode('無法更新廠商金資訊：' . $e->getMessage(), JSON_UNESCAPED_UNICODE);
    exit;
}

// 告知成功，並返回員工與餐廳管理頁
$link = NULL;
echo json_encode('廠商金資訊更新成功。', JSON_UNESCAPED_UNICODE);
exit;
